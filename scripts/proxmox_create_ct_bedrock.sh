...
# Generate a random 16-char alphanumeric password for the CT root account.
# The password is printed at the end – change it after first login.
...
pct create "$CTID" "${STORE}:vztmpl/${TPL}" \
  -hostname mc-bedrock -cores 2 -memory "$MEM" -swap 512 \
  -rootfs local-lvm:8 -net0 name=eth0,bridge=vmbr0,ip=dhcp \
  -features keyctl=1,nesting=1 -unprivileged 0 \
...
echo "CT $CTID ready."
echo "⚠️  Root password for CT $CTID: ${CT_PASSWORD}"
echo "     Change it immediately after first login: pct exec $CTID -- passwd root"
